...
// fakeHealthClient is a fake implementation of grpc_health_v1.HealthClient for testing.
...
func (f *fakeHealthClient) Check(ctx context.Context, in *grpc_health_v1.HealthCheckRequest, opts ...grpc.CallOption) (*grpc_health_v1.HealthCheckResponse, error) {
    // Emulate setting the peer if the grpc.Peer option is provided.
    if f.err != nil {
        return nil, f.err
    }
...
func (f *fakeHealthClient) Watch(ctx context.Context, in *grpc_health_v1.HealthCheckRequest, opts ...grpc.CallOption) (grpc_health_v1.Health_WatchClient, error) {
    // Return an unimplemented error for the Watch functionality.
    return nil, status.Error(codes.Unimplemented, "Watch is not implemented in fakeHealthClient")
}
...
func TestProbeInvalidURL(t *testing.T) {
    // This test checks that ProbeGRPC returns false when given an invalid URL.
    ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
    defer cancel()
    registry := prometheus.NewRegistry()
    // Pass an invalid URL that will fail in url.Parse.
    result := ProbeGRPC(ctx, "://bad", config.Module{}, registry, promslog.NewNopLogger())
    if result {
        t.Fatal("Expected ProbeGRPC to return false for an invalid URL")
    }
}

func TestGRPCHealthCheckClient(t *testing.T) {
    // This test directly tests gRPCHealthCheckClient.Check by using our fakeHealthClient.
    // First, simulate a successful health check response.
    fakeClient := &fakeHealthClient{status: grpc_health_v1.HealthCheckResponse_SERVING}
    client := gRPCHealthCheckClient{
        client: fakeClient,
        // The client.conn field is only used for closing so it can be a dummy pointer.
        conn: &grpc.ClientConn{},
    }
    ctx := context.Background()
    ok, code, _, statusStr, err := client.Check(ctx, "dummyService")
    if err != nil {
        t.Fatalf("Unexpected error during health check: %v", err)
    }
    if !ok {
        t.Fatal("Expected health check to succeed")
    }
    if code != codes.OK {
        t.Fatalf("Expected code codes.OK, but got %v", code)
    }
    if statusStr != grpc_health_v1.HealthCheckResponse_SERVING.String() {
        t.Fatalf("Expected status %q, got %q", grpc_health_v1.HealthCheckResponse_SERVING.String(), statusStr)
    }

    // Now simulate an error response from the health check.
    fakeErr := status.Error(codes.NotFound, "service not found")
    fakeClient.err = fakeErr
    ok, code, _, _, err = client.Check(ctx, "dummyService")
    if ok {
        t.Fatal("Expected health check to fail")
    }
    if code != codes.NotFound {
        t.Fatalf("Expected code NotFound, got %v", code)
    }
}